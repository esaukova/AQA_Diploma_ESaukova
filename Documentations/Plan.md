...
|          |                                               |                                |                             ||   
| ТС 10    | POST request "купить" с пустым значением поля "status"         |1.Отправить POST запрос на эндпоинт http://localhost:8080/api/v1/pay с валидными данными и пустым значением поля "status"|Статус-код ответа 400  Bad Request.|
|          |                                               |                                |                             ||   
...
|          |                                               |                                |                             ||   
| ТС 16    | POST request "купить в кредит" с пустым значением поля "status"         |1.Отправить POST запрос на эндпоинт http://localhost:8080/api/v1/credit с валидными данными и пустым значением поля "status"|Статус-код ответа 400  Bad Request.|
...
| Идентификационный  номер | Название | Шаги | Ожидаемый результат|
|----------|----------|----------|-----------|
| ТС 41    | Оплата тура в кредит с валидными данными и статусом карты DECLINED         |1.Ввести номер карты со статуом DECLINED: 4444 4444 4444 4442| Система выдаёт сообщение "Ошибка! Банк отказал в проведении операции." Появляется запись в БД: credit_request_entity со статусом DECLINED.|
